...
class ScreeningResponse(models.Model):
    session = models.ForeignKey(
        ScreeningSession, on_delete=models.CASCADE, related_name="responses"
    )
...
class GradingConfig(models.Model):
    ...
    grading_example = '{"grade_boundaries": {"A": 10, "B": 20, "C": 30, "D": 40, "F": 100}}'
    rules = models.JSONField(
        default=dict,
        blank=True,
        help_text=f"Psychiatrist-defined grading rules. Example: {grading_example}",
    )
