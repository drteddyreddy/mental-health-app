...
from screening.utils import calculate_score, get_recommendations, get_severity
...
def pdf_report(request, campaign_id):
    ...
    elements.append(
        Paragraph(f"Completion Rate: {rate}% ({sessions.count()}/{total})", styles["Normal"])
    )
    elements.append(Spacer(1, 12))
    ...
    elements.append(
        Paragraph("Scale: 0–4 Minimal | 5–9 Mild | 10–14 Moderate | 15+ Severe", styles["Normal"])
    )
    elements.append(
        Paragraph("Aggregate data only. No individual results disclosed.", styles["Normal"])
    )
...
def employee_lookup(request):
    ...
    try:
        employee = Employee.objects.get(company=company, anonymous_code=code, is_active=True)
...
def change_password(request):
    ...
    if len(new) < 8:
        return Response(
            {"error": "Password must be at least 8 characters"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def csv_report(request, campaign_id):
    ...
    campaign = get_object_or_404(Campaign, pk=campaign_id, company=company)
    sessions = ScreeningSession.objects.filter(
        campaign=campaign, completed_at__isnull=False
    ).select_related("employee")

    response = HttpResponse(content_type="text/csv")
    response["Content-Disposition"] = (
        f'attachment; filename="report_{campaign.name.replace(" ", "_")}.csv"'
    )
