...
from screening.utils import calculate_score
...
class IsPsychiatrist(permissions.BasePermission):
    def has_permission(self, request, view):
        return request.user.is_authenticated and (
            ...
            or getattr(getattr(request.user, "profile", None), "role", None) == "psychiatrist"
        )
...
def psy_dashboard(request):
    if (
        ...
        and getattr(getattr(request.user, "profile", None), "role", None) != "psychiatrist"
    ):
...
def psy_grading_config(request, company_id):
    ...
    serializer = GradingConfigSerializer(
        config, data={"rules": request.data.get("rules", {})}, partial=True
    )
