...
from .forms import CompanyForm, CSVUploadForm, CampaignForm
...
def dashboard(request):
    ...
    sessions = ScreeningSession.objects.filter(campaign__company=company, completed_at__isnull=False)
...
def employee_list(request):
    ...
    employees = Employee.objects.filter(company=company, is_active=True).order_by('department', 'name')
...
def campaign_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Campaign "{campaign.name}" created! Anonymous links generated.')
...
def screening_view(request, link_id):
    ...
    questions = Question.objects.filter(questionnaire__in=questionnaires).order_by('questionnaire__order', 'order')
...
def employee_results(request):
    ...
    if code_search:
        try:
            employee = Employee.objects.get(company=company, anonymous_code=code_search, is_active=True)
...
def pdf_report(request, campaign_id):
    ...
    response['Content-Disposition'] = f'attachment; filename="report_{campaign.name.replace(" ", "_")}.pdf"'
    ...
    elements.append(Paragraph(f'Completion Rate: {completion_rate}% ({sessions.count()}/{total_employees})', styles['Normal']))
    elements.append(Spacer(1, 12))
    ...
    elements.append(Paragraph('Severity Scale: 0-4 Minimal | 5-9 Mild | 10-14 Moderate | 15+ Severe', styles['Normal']))
    elements.append(Paragraph('This report contains aggregate data only. No individual employee results are disclosed.', styles['Normal']))
