...
from .models import (
    ...
    Question, ScreeningSession, ScreeningResponse, UserProfile, GradingConfig,
)
from .utils import calculate_score, get_severity, get_recommendations
...
class ModelTests(TestCase):
    def setUp(self):
        ...
        self.session = ScreeningSession.objects.create(campaign=self.campaign, employee=self.employee)
    ...
    def test_grading_config_str(self):
        cfg = GradingConfig.objects.create(company=self.company, rules={'grade_boundaries': {'A': 10}})
...
class UtilTests(TestCase):
    def setUp(self):
        ...
        self.q1 = Question.objects.create(questionnaire=self.qnr, text='Q1', order=1, max_score=3)
        self.q2 = Question.objects.create(questionnaire=self.qnr, text='Q2', order=2, max_score=3)
        self.campaign = Campaign.objects.create(company=self.company, name='C1', start_date='2026-01-01', end_date='2026-03-31')
        self.campaign.questionnaires.add(self.qnr)
        self.session = ScreeningSession.objects.create(campaign=self.campaign, employee=self.employee)
    ...
    def test_reverse_scoring(self):
        qnr = Questionnaire.objects.create(name='CBI-W', max_score=28, metadata={'reverse_items': [0]})
        q = Question.objects.create(questionnaire=qnr, text='Q', order=1, max_score=4)
        campaign = Campaign.objects.create(company=self.company, name='C2', start_date='2026-01-01', end_date='2026-03-31')
...
class APITests(APITestCase):
    def setUp(self):
        ...
        self.employee = Employee.objects.create(company=self.company, name='Bob', department='Sales')
        ...
        self.campaign = Campaign.objects.create(company=self.company, name='Q1', start_date='2026-01-01', end_date='2026-03-31')
        self.campaign.questionnaires.add(self.qnr)
        self.session = ScreeningSession.objects.create(campaign=self.campaign, employee=self.employee)

    def _auth(self):
        resp = self.client.post('/api/auth/login/', {'username': 'hr', 'password': 'pass1234'}, content_type='application/json')
        token = resp.json()['access']
        self.client.defaults['HTTP_AUTHORIZATION'] = f'Bearer {token}'

    def test_register(self):
        data = {'username': 'newhr', 'email': 'hr@test.com', 'password': 'testpass123', 'company_name': 'NewCo'}
    ...
    def test_employee_create(self):
        self._auth()
        resp = self.client.post('/api/employees/', {'name': 'Charlie', 'department': 'IT', 'designation': 'Eng'}, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
    ...
    def test_screening_submit(self):
        resp = self.client.post(f'/api/screening/{self.session.unique_link_id}/', {
            'responses': [
    ...
    def test_employee_upload_csv(self):
        ...
        file = SimpleUploadedFile('employees.csv', buf.getvalue().encode('utf-8-sig'), content_type='text/csv')
...
class AuthEdgeCaseTests(APITestCase):
    def test_register_missing_fields(self):
        resp = self.client.post('/api/auth/register/', {'username': 'nopass'}, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('required', resp.json().get('error', ''))

    def test_register_duplicate_username(self):
        User.objects.create_user(username='taken', password='pass1234')
        data = {'username': 'taken', 'email': 'a@b.com', 'password': 'pass1234', 'company_name': 'C'}
        resp = self.client.post('/api/auth/register/', data, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_login_wrong_password(self):
        ...
        resp = self.client.post('/api/auth/login/', {'username': 'u', 'password': 'wrong'}, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_token_refresh_invalid(self):
        resp = self.client.post('/api/auth/token/refresh/', {'refresh': 'invalid'}, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_change_password_wrong_old(self):
        ...
        resp = self.client.post('/api/auth/login/', {'username': 'u', 'password': 'old1234'}, content_type='application/json')
        token = resp.json()['access']
        resp = self.client.post('/api/auth/change-password/', {'old_password': 'wrong', 'new_password': 'new12345678'},
                                content_type='application/json', HTTP_AUTHORIZATION=f'Bearer {token}')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn('incorrect', resp.json().get('error', ''))

    def test_change_password_unauthenticated(self):
        resp = self.client.post('/api/auth/change-password/', {'old_password': 'x', 'new_password': 'y'},
                                content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)
...
class ScreeningEdgeCaseTests(APITestCase):
    ...
    def test_screening_post_invalid_link(self):
        resp = self.client.post(f'/api/screening/{uuid.uuid4()}/', {'responses': []}, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_404_NOT_FOUND)

    def test_screening_submit_empty_responses(self):
        resp = self.client.post(f'/api/screening/{self.session.unique_link_id}/', {'responses': []},
                                content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
...
class EmployeeEdgeCaseTests(APITestCase):
    ...
    def test_employee_list_excludes_inactive(self):
        self._auth()
        Employee.objects.create(company=self.company, name='Charlie', department='IT', is_active=False)
        resp = self.client.get('/api/employees/')
        results = resp.json().get('results', resp.json())
...
class CampaignEdgeCaseTests(APITestCase):
    ...
    def test_campaign_create_without_questionnaire_ids(self):
        self._auth()
        resp = self.client.post('/api/campaigns/', {
            'name': 'NoQs', 'start_date': '2026-01-01', 'end_date': '2026-03-31',
        }, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

    def test_campaign_sessions_zero_completion_rate(self):
        self._auth()
        campaign = Campaign.objects.create(company=self.company, name='Empty', start_date='2026-01-01', end_date='2026-03-31')
    ...
    def test_campaign_access_other_company_returns_404(self):
        ...
        other_campaign = Campaign.objects.create(company=other_company, name='OtherC', start_date='2026-01-01', end_date='2026-03-31')
    ...
    def test_campaign_create_creates_sessions_for_active_employees(self):
        self._auth()
        Employee.objects.create(company=self.company, name='ActiveEmp', department='D', is_active=True)
        Employee.objects.create(company=self.company, name='InactiveEmp', department='D', is_active=False)
        resp = self.client.post('/api/campaigns/', {
            'name': 'WithSessions', 'start_date': '2026-01-01', 'end_date': '2026-03-31',
            'questionnaire_ids': [self.qnr.id],
        }, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
...
class PsychiatristEdgeCaseTests(APITestCase):
    ...
    def _auth_psy(self):
        resp = self.client.post('/api/auth/login/', {'username': 'psy', 'password': 'pass1234'}, content_type='application/json')
        self.client.defaults['HTTP_AUTHORIZATION'] = f'Bearer {resp.json()["access"]}'
    ...
    def test_psy_can_update_company_questionnaires(self):
        ...
        resp = self.client.put(f'/api/psy/companies/{self.hr_company.id}/',
                               {'assigned_questionnaire_ids': [qnr.id]}, content_type='application/json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
    ...
    def test_psy_grading_config_put(self):
        self._auth_psy()
        resp = self.client.put(f'/api/psy/companies/{self.hr_company.id}/grading/',
